        ...
        <c:if test="${ not empty list }">
          <c:forEach items="${ list }" var="apply" varStatus="status">
            <tr>
              ...
              <td><a href="/User/Mypage/Apply/${ apply.sendnum }">${ apply.restitle }</a></td>
              ...
              <c:choose>
                ...
                <c:when test="${ apply.pass_code == null }">
...
<script>
  /* window.onload = function(){
	  $.ajax({
		  url:"/User/Mypage/Apply/list",
		  type:"GET",
		  success: function(applyList){
			  const view = document.querySelector('#applyListView');
			  let html = "";
			  for(let i = 0; i < applyList.length; i++){
				  html += '<tr><td>'+(i+1)+'</td><td><a href="/User/Mypage/Apply/'+applyList[i].sendnum+'">'+applyList[i].restitle+'</a></td><td><a href="#">'
				  +applyList[i].comname+'</a></td><td>'+applyList[i].reg_date+'</td>';
				  if(applyList[i].pass_code != null && applyList[i].pass_code == 1){ html += '<td class="green">합격</td>' }
				  else if(applyList[i].pass_code != null && applyList[i].pass_code == 0){ html += '<td class="red">불합격</td>' }
				  else{ html += '<td class="orange">심사중</td>' };
				  html += '</tr>';
			  }
			  view.innerHTML = html;
		  },
  ...
  function filter(pass_code, callback){
	  $.ajax({
		  url: "/User/Mypage/Apply/filter/"+pass_code,
		  type: "GET",
		  success: (filterList) =>{
			  let html = "";
			  ...
			  else if(pass_code == '0'){ html = "<tr><td colspan='6'>불합격된 이력서가 없어요!😄</td></tr>"; }
