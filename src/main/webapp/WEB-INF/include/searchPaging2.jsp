...
<div class="pagination">
    <c:if test="${commPagingHelper.hasPreviousPageGroup()}">
        <a href="?keyword=${keyword}&commPage=1" class="page-arrow arrow-first">⇤</a>
        <a href="?keyword=${keyword}&commPage=${commPagingHelper.previousPageGroupStart}" class="page-arrow arrow-prev">←</a>
    </c:if>

    <c:forEach var="i" begin="${commPagingHelper.startPage}" end="${commPagingHelper.endPage}">
        <c:choose>
            <c:when test="${i == commPagingHelper.nowPage}">
                <span class="now">${i}</span>
            </c:when>
            <c:otherwise>
                <a href="?keyword=${keyword}&commPage=${i}" class="page-link">${i}</a>
            </c:otherwise>
        </c:choose>
    </c:forEach>

    <c:if test="${commPagingHelper.hasNextPageGroup()}">
        <a href="?keyword=${keyword}&commPage=${commPagingHelper.nextPageGroupStart}" class="page-arrow arrow-next">→</a>
        <a href="?keyword=${keyword}&commPage=${commPagingHelper.totalPages}" class="page-arrow arrow-last">⇥</a>
    </c:if>
</div>
