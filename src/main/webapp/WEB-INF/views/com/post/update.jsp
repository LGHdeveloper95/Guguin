...
<main>
    ...
    <div>
        <form action="/post/update" id="postWriteForm" method="POST">
            ...
            <table>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <a class="noa btn btn-success" id="submit" href="/post/update">수정</a>
                    </td>
                </tr>
                </thead>
                <tr>
...
<script>
    ...
    fetch("/post/update-backup/${recnum}", {
            method: "GET"
        }
    ).then(response => response.json())
        .then(map => {
            let skills = map.post.skills.split(",");
            ...
            document.getElementsByName("rectitle")[0].value = map.post.rectitle;
            document.getElementsByName("subtitle")[0].value = map.post.subtitle;
            document.getElementsByName("gcontent")[0].value = map.post.gcontent;
            document.getElementsByName("deadline")[0].value = map.post.deadline.substring(0, 10);
            document.getElementsByName("start_date")[0].value = map.post.start_date.substring(0, 10);
            document.getElementsByName("career_code")[map.post.career_code - 1].click();
            document.getElementsByName("edu_code")[map.post.edu_code - 1].click();
            const getLicenseList = map.post.license.split(',');
            if (getLicenseList) {
            ...
            clicked += '<input type="hidden" name="gugun_code" value="' + map.post.gugun_code + '"/>'
            clicked += '<div class ="g_click" value="' + map.post.gugun_code + '">' + map.region.sido + " " + map.region.gugun + '</div>';
            let check = true;
    ...
    document.getElementById('submit').addEventListener('click', function () {
        ...
        if ($('input[name=start_date]').val() == today || $('input[name=start_date]').val() < today || $('input[name=deadline]').val() < $('input[name=start_date]').val()) {
            alert('시작일을 설정해주세요.');
            $('input[name=start_date]').focus();
            return false
        }
        if ($('input[name=deadline]').val() == today || $('input[name=deadline]').val() < today) {
            alert('마감일을 설정해주세요.');
