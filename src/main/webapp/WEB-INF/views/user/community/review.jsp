  ...
  <main>
    <%@include file="/WEB-INF/include/mypageSide.jsp" %>
	  ...
	  <h5>내가 쓴 기업리뷰</h5>
		...
		<c:if test="${ not empty comReviewList }">
		  <c:forEach items="${ comReviewList }" var="review" varStatus="i">
		    <tr>
		      <td>${ i.index + 1 }</td>
		      <td><a href="/com-info/${ review.comid }">${ review.comname }</a></td>
		      <td>${ review.commented }</td>
		      <td>${ review.average_val }</td>
		    </tr>
		  </c:forEach>
		</c:if>
		<c:if test="${ empty comReviewList }">
		  <tr><td colspan="3">리뷰를 쓰지 않았어요</td></tr>
		</c:if>
		</table>
	  </div>
  </main>
