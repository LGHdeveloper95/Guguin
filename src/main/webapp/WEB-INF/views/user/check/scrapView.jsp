    ...
    <%@include file="/WEB-INF/include/mypageSide.jsp" %>
	  ...
	  <h5>스크랩공고</h5>
		...
		<c:if test="${ not empty scrapList }">
		  <c:forEach items="${ scrapList }" var="scrap" varStatus="i">
		    <tr>
		      <td>${ i.index + 1 }</td>
		      <td><a href="/post/view/${ scrap.recnum }">${ scrap.rectitle }</a></td>
		      <td><a href="/com-info/${ scrap.comid }">${ scrap.comname }</a></td>
		      <td>${ scrap.deadline }</td>
		      <td>${ scrap.views }</td>
		    </tr>
		  </c:forEach>
		</c:if>
		<c:if test="${ empty scrapList }">
		  <tr><td colspan="3">스크랩한 공고가 없어요!</td></tr>
		</c:if>
		</table>
	  </div>
  </main>
