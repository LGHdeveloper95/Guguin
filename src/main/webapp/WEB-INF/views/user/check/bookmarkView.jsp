...
<head>
  ...
  .table{
	...
	.table td:first-of-type{ max-width: 1px; }
    ...
    <%@include file="/WEB-INF/include/mypageSide.jsp" %>
	  ...
	  <h5>관심기업</h5>
		...
		<c:if test="${ not empty bookmarkList }">
		  <c:forEach items="${ bookmarkList }" var="bookmark" varStatus="i">
		    <tr>
		      <td>${ i.index + 1 }</td>
		      <td><a href="/com-info/${ bookmark.comid }">${ bookmark.comname }</a></td>
		      <td>${ bookmark.average_val }</td>
		    </tr>
		  </c:forEach>
		</c:if>
		<c:if test="${ empty bookmarkList }">
		  <tr><td colspan="3">북마크한 회사가 없어요!</td></tr>
		</c:if>
		</table>
	  </div>
  </main>
